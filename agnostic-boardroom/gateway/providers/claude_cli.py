...
**Harness-agnosticism, honestly scoped**: `ClaudeCliProvider.run` is
...
when this module was first imported, silently ignoring a later
...
inside the method itself avoids that class of stale-env-var bug entirely.

This is the one provider adapter under `gateway/providers/`, moved here
(verbatim logic, unchanged behavior) from `agents/model_runner.py` when the
`gateway/` routing library was added -- see `gateway/router.py`'s own
docstring for why a stateless router, not a persistent service, is the
right shape here.
"""
