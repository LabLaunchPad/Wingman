...
1. **No persistent runtime.** Claude Code has no long-running multi-process graph server. A subagent is a markdown file invoked transiently through the Task tool inside one session; it does not persist between calls the way a LangGraph node does. This rule means no always-on agent process — it does not mean no local persistence: file-backed state opened on demand (JSONL, SQLite) and session-scoped or stdio-launched local MCP servers are both compatible with it, since neither requires a background daemon. See `docs/status/DATABASE.md`'s "Planned: an MCP server" section for the evidence-gated design that already assumes this reading.
...
| **Specialists** | Narrow sub-roles (the 56+-role candidate catalog in `docs/roadmap/AGENT-ROSTER.md`, uncapped and growing as evidenced need arises). | Grows 0 → N, uncapped | Only created by `/wingman:evolve`, after a department lead shows *repeated* (2+) friction in one narrow area |
...
Every checkpoint runs all seven seats in parallel and consolidates them into one plain-language, grouped-header verdict (see `plugins/wingman/commands/adaptive/boardroom.md`).
...
Every agent/manager/department-lead/specialist template declares a `permissions:` frontmatter field: one of `read`/`write`/`approve`/`execute`/`deploy`. `approve` is exclusive to Boardroom seats (mechanically enforced by `validate-structure.mjs`); department leads and specialists get `read`+`write` (they produce); Management Board managers get `read` (they coordinate and recommend, never approve); `deploy` is reserved for `dept-devops`-dispatched work, and only after a Boardroom approval has already happened. As of 2026-07-27, these map onto the 5-tier model in `plugins/wingman/references/permission-model.md` (Read only / Draft and propose / Scoped write / Conditional action / Break-glass — restated for human readers in root `POLICIES.md`, adopted from a founder-supplied blueprint review). This is still a documentation-and-consistency field for most of its range, not a fully runtime-enforced permission check — it exists to make autonomy boundaries explicit and auditable as the agent population grows.

**Boardroom CISO review, 2026-07-19** (prompted by a founder-shared Google SAIF 2.0 "Secure Agents" risk-map cross-check): confirmed this gap directly — an agent scoped `permissions: write` but granted the `Bash` tool (the department-lead template's own recommendation for "a department that writes code") has no mechanical barrier stopping a deploy-class action; the only real runtime boundary is each agent's `tools:` allowlist. Verdict: `GO_WITH_CONCERNS`, accepted as a known limit — Claude Code's own hook surface has no event that intercepts "this agent is about to do something outside its stated `permissions:`" (confirmed directly: every one of Wingman's own hooks, and an independent second plugin's hooks, read only `cwd`/`tool_name`/`tool_input`/`session_id` from their payload — no agent-identity field is exposed to intercept on).

**Partially closed, 2026-07-27**: rather than building an identity-based enforcement layer (confirmed above to be impossible against the real hook payload shape), `plugins/wingman/hooks/deploy-approval-gate.mjs` enforces the Level 3/4 boundary — the one that matters most — against a different, real substrate: it blocks deploy-class Bash commands unless the most recent Build-stage checkpoint in `.wingman/checkpoints.jsonl` carries a clean Boardroom `GO`, reusing `dod-structural-gate.mjs`'s own `checkBoardroomVerdictClean()`. This closes "did this project's Boardroom actually approve a deploy-class action" without needing to know which agent is asking. What remains a disclosed, accepted limit: an agent below that boundary (Level 0-2) still has no mechanical barrier beyond its own `tools:` allowlist — revisit if a real incident, not a hypothetical one, shows that allowlist alone is insufficient.

### Checkpoint audit log

Every boardroom run appends one line to `.wingman/checkpoints.jsonl` at the project root (git-committed, human-readable) — see `docs/status/DATABASE.md` for the full schema, the `schema_version: 2` migration note, and the old→new seat-name mapping from the 5-seat schema. As of `schema_version: 4`, each entry also gets a companion file at `.wingman/checkpoint-details/<checkpoint_id>.md` holding every seat's full, unabridged verdict — the reversible-compression counterpart to `plain-language-checkpoint`'s one-line summaries, retrievable later via `/wingman:boardroom expand <checkpoint_id>` (see `docs/status/DATABASE.md`'s `schema_version: 3 → 4` migration note). As of `schema_version: 5`, every one of the 14 pipeline stages records its own checkpoint entry — see §4d.
...
founder-visible checkpoints total**, up from 3. See `docs/status/DATABASE.md`'s `schema_version: 4 → 5`
...
stage names encoded in `checkpoints.jsonl`'s schema are untouched. See `docs/status/PROJECT.md`'s decisions
...
The specialist candidate catalog (56+ roles, growing as new candidate roles are added to `docs/roadmap/AGENT-ROSTER.md` — e.g. React/Python/RAG/LLM Integration/RL/Prompt Engineer, added 2026) lives there as **candidates**, organized under their department. None of them are files until `/wingman:evolve` promotes one:

- A department lead repeatedly hits friction in one narrow sub-task (2+ occurrences logged via `/wingman:learn`, surfaced in `docs/history/retros.md`, or visible as repeated `GO_WITH_CONCERNS`/`NO_GO` on the same topic in `.wingman/checkpoints.jsonl`).
...
`LEARNINGS.md`, `docs/history/retros.md`, and `docs/status/PROJECT.md`'s decisions log carry a
`<!-- wingman:log type=... category=... status=... -->` marker above every entry; `docs/history/GAPS.md`
...
coverage benchmark** scored from `docs/status/AGENT-WEAKNESS-BENCHMARK.md`) — deliberately not a
...
docs and nothing else. It has no bearing on `docs/status/CROSS-USER-LEARNING.md`'s ruled-out cross-user/
...
`docs/history/architecture-audit-2026-07-15.md`'s Proven finding #4: this plugin is **not** portable to a different coding-agent harness (Codex CLI, Gemini CLI, OpenHands, Cline) as a whole, and that should be stated plainly rather than left as an unexamined assumption behind a general "agent-agnostic" label.
...
- `AskUserQuestion` (`commands/adaptive/boardroom.md`) — already self-disclosed as unavailable in headless/print-mode sessions.
...
**Follow-up executed, same week (2026-07-23)**: the deferred flatten was carried out as its own change.
`plugins/wingman/skills/<category>/<name>/SKILL.md` → `plugins/wingman/skills/<name>/SKILL.md` for all
40 skills (`git mv`, preserving history), ~90 files' path references updated repo-wide,
`generate-harness-adapters.mjs`'s `listSkills()` fixed (it hardcoded the 2-level nesting and would
have silently found zero skills otherwise), both generators regenerated and `--check` clean. A real
regression surfaced and was fixed during this pass: two test files
(`tests/hooks-integration/hooks-integration.test.mjs`, `tests/ponytail-integration/ponytail-integration.test.mjs`)
independently hardcoded `['category', 'name']` tuples to build skill paths, outside the original
sed's scope (`plugins/wingman/`, `evals/cases/`, root docs — not `tests/`) — caught by a full
`node --test` run (28 then 13 failures), fixed, back to 173/173. Also found and removed 7 now-empty
per-category `README.md` stubs (boilerplate pointers to this same `AGENTS.md`, no unique content).
Live-install re-verified: OpenCode still discovers all 40 skills from the flat
`shared/.agents/skills/` copy; Codex CLI's `codex plugin add` cache now genuinely mirrors the flat
structure (41 entries: 40 skills, zero category directories) and still discovers everything
correctly — confirming the actual point of this follow-up (cache-path alignment) worked as intended.
...
mindwalk/swarm-intelligence/hybrid-framework pitches, all logged in `docs/roadmap/AGENT-ROSTER.md`'s
...
**All 6 harnesses now have an adapter.** What remains open, tracked in `docs/status/PROJECT.md`, not
...
- **v3** (built, verified): `skills/evolve-promotion` — the concrete mechanism behind `/wingman:evolve`, gathering signal from `LEARNINGS.md`, `docs/history/retros.md`, and `.wingman/checkpoints.jsonl`, requiring 2+ genuine occurrences, and writing promoted specialists to the founder's own project (never Wingman's plugin directory). Behaviorally exercised — `evals/cases/evolve-promotion.md` (`verified`, positive + negative cases). The 56-role catalog fills in organically and differently per project as this runs — never as a fixed upfront set.
- **v3.1 (built, verified)**: `commands/adaptive/launch.md` and `commands/adaptive/hotfix.md` — each has a dedicated eval case now (`evals/cases/launch.md`, `evals/cases/hotfix.md`), both `verified` as of 2026-07-08 (launch: docs-skip + docs-write; hotfix: single- and multi-hypothesis bugs).
...
- **v12 (maintenance, recommended by the project's own rules)**: brought the plugin to a **fully check-clean** state and reconciled the stale status doc. (1) The 5 skills that violated the skill-anatomy standard (`ponytail-debt-harvesting`, `platform-native-reference`, `verification-loop`, `interview-one-question-at-a-time`, `evidence-gated-catalog`) now carry `Use when…` trigger clauses in their `description` (also fixing the "description trap" auto-routing risk) and `interview-one-question-at-a-time` gained its missing `## Verification` section — `validate-structure.mjs` now emits **zero** warnings. (2) `docs/status/PROJECT.md` was stale (claimed "13 commands / 10 skills / all 10 eval cases") and now matches reality (16 commands, 25 skills, 25 eval cases, v9–v11 recorded) — per the project's "stale status is worse than no status doc" rule. 84 tests passing; `validate-structure` and `check-repo-consistency` both PASS with no warnings.
...
- **v20 (14-stage pipeline, founder-approved reversal of v14's bundling decision)**: expanded the 7-stage pipeline to 14 stages / 12 individual pre-build Boardroom checkpoints (up from 1 bundled "Planning Milestone" checkpoint) — see §4d for the full stage list and rationale. 7 new pipeline commands (`research-synthesis.md`, `personas-jobs.md`, `journey-mapping.md`, `information-architecture.md`, `wireframes.md`, `visual-design-system.md`, `prototype-usability.md`) plus 1 new adaptive command (`post-launch.md`). `implementation-planning.md`'s bundled `"bundle": "planning-milestone"` checkpoint is removed — it now records only its own solo checkpoint, and its "Gather" step reads all 11 prior stage docs instead of 4. `checkpoints.jsonl` bumped to `schema_version: 5` (see `docs/status/DATABASE.md`'s migration note). `skills/traceability-linking` and `check-traceability.mjs` extended with 7 new ID prefixes (`RS-`/`PJ-`/`JM-`/`IA-`/`WF-`/`VS-`/`PT-`). `## Planning Milestone checkpoint` renamed to `## Implementation Planning checkpoint` in both `implementation-planning.md` and `hooks/dod-structural-gate.mjs`'s marker constant.

- **v21 (6-field skill contract across all 40 skills)**: extended `validate-structure.mjs`'s skill-anatomy check from the 3-concept triad (Rationalizations/Red Flags/Verification) to the full 6-field contract adopted from a founder-supplied blueprint review — Trigger / Inputs / Procedure / Output schema / Stop condition / Escalation. Trigger (frontmatter `description`'s "Use when..." clause) and Procedure ("Core Workflow") were already mechanically covered; the 3 genuinely new fields (Inputs, Output, Escalation) were hand-authored into all 40 skills across 5 waves, each grounded in that skill's own real content rather than templated — a templated pass would have produced 40 skills all saying "Inputs: the task at hand," defeating the point of adopting the contract. `validate-structure.mjs` now emits zero anatomy warnings repo-wide. Fifth of 6 planned PRs harvesting genuine gaps from the founder-supplied blueprint review (see `docs/status/PROJECT.md`'s decisions log for the full series).
...
decisions before any code was written, it directly reverses three of them: `docs/status/DATABASE.md`'s
...
rewrite. See `docs/status/PROJECT.md`'s decisions log for the full record.
...
1. **Data & Schema (done)** — `core/state_schema.py`: Pydantic models (`ProjectState`,
   ...
   ported from `docs/status/DATABASE.md`'s and the threat-register/debt-ledger docs' real, already-shipped
   shapes — not a speculative new schema. 7 tests pass, two copied verbatim from `docs/status/DATABASE.md`'s
...
real live-model tests run explicitly. See `docs/status/PROJECT.md`'s decisions log for the full record of
...
see `docs/status/PROJECT.md`'s decisions log for the exact exchange. `agents/boardroom_engine.py` composes
...
- The MCP state-store server documented in `docs/status/DATABASE.md` (deliberately deferred — see that document's "Why no server yet").
