...
CI's `validate.yml` runs five further Layer-1 checks beyond the two scripts above (this list itself had silently drifted — `validate-wkos.mjs` shipped in PR3 of the AI Engineering Operating System build with no entry here; fixed in the same pass that added `validate-engines.mjs`, rather than repeating the omission):
- **`node plugins/wingman/scripts/validate-wkos.mjs`** — the WKOS document contract: every document has a real producer, or it is a template — the mechanical guard against the ~130-doc structure becoming dead weight (`docs/status/PROJECT.md`, 2026-07-22).
- **`node plugins/wingman/scripts/validate-engines.mjs`** — the engine ownership contract (PR7 of the AI Engineering Operating System build; reorganized 2026-07-30 into 22 EngineOS engines): every command/skill/hook/reference has exactly one engine owner, no orphans — see `docs/status/ENGINES.md`.
...
- **`node --test tests/*/*.test.mjs`** — the `node --test` suite (currently 345 tests across 16 files in 4 directories). Added 2026-07-27: the suite had existed for months but **no CI workflow ran it**, so a broken test could reach `main` as long as nobody ran it by hand — the same "only enforced when a maintainer remembers" failure class as the conflict-marker incident. Scoped to `tests/` deliberately (a bare `node --test` also recurses into `vendor/`'s pinned upstream tests), and the glob is left unquoted so the *shell* expands it, since Node 20 (what CI pins) and Node 22 interpret glob/directory arguments to `--test` differently.
