...
[`docs/status/ARCHITECTURE.md`](ARCHITECTURE.md) §3–§5a:

| Layer | What it does | Where it's defined |
|---|---|---|
| **Boardroom** (fixed, 7+1 seats) | Reviews and gates — never writes code, only renders a plain-language go/no-go verdict at a checkpoint. | `docs/status/ARCHITECTURE.md` §4, `plugins/wingman/agents/boardroom-*.md` |
| **Management Board** (grows 0→9) | Coordinates department-lead execution once a project is complex enough (3+ active conditionally-created department leads). Never renders Boardroom verdicts. | `docs/status/ARCHITECTURE.md` §5a, `plugins/wingman/skills/management-board-activation` |
| **Department leads** (grows 0→8) | Build-time workers, one per corporate department, created lazily per real project need. | `docs/status/ARCHITECTURE.md` §5, `plugins/wingman/skills/department-lead-activation` |
| **Specialists** (uncapped) | Narrow sub-roles, promoted one at a time only after 2+ evidenced occurrences of repeated friction. | `docs/roadmap/AGENT-ROSTER.md`, `plugins/wingman/skills/evolve-promotion` |

The gate rule and human-escalation mapping (which risk tier routes to whom) are in
`docs/status/ARCHITECTURE.md` §4's "Gate rule" and "Human Escalation Framework" subsections.
...
Wingman has **no persistent runtime and no request traffic** (`docs/status/ARCHITECTURE.md` §2), so its
...
| `docs/history/*.md` (point-in-time audits: `architecture-audit-2026-07-15.md`, `enhancement-review-2026-07-15.md`, `audit-reorg-2026-07-20.md`) | **Historical record.** Each is a dated snapshot; not living docs. | No owner needed to keep current — they're intentionally frozen at their audit date. `docs/history/audit/` (numbered dossier) is the *current* audit convention; new audits go there. | Never updated in place — a new audit gets a new dated file or a new numbered `docs/history/audit/NN-*.md` entry. |
