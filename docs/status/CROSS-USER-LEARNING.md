...
- **No phone-home.** Wingman never transmits anything from a founder's project automatically. A founder's code and business details are sensitive; the plugin has no runtime service and shouldn't grow one just for this (`docs/status/DATABASE.md`'s "Why no server yet" and the `engineering-minimalism` skill both apply). Everything here is flat files and explicit human action.
...
- **The founder's own loops already exist.** `LEARNINGS.md`, `docs/history/retros.md`, and `/wingman:evolve` already capture and act on lessons *within* one project. This doc is only about the extra step of promoting a genuinely general lesson *up* to Wingman.
...
- Open it as a GitHub issue (or PR against `docs/roadmap/AGENT-ROSTER.md` / a skill) on the Wingman repo, for a maintainer to review.
...
Wingman is built by running Wingman on itself. Bugs and lessons that surface from *exercising* the plugin (not from a founder hitting them) have a durable home: `docs/status/PROJECT.md`'s decisions log, and — for anything an audit surfaces — `docs/status/REGRESSION-CHECKLIST.md`. This session's own history is the proof this works: the inert-hook bug, the model-tier drift, and the attribution error were all found by dogfooding and are now both fixed and defended against mechanically. Every eval run's findings getting a durable home is itself a cross-"usage" learning loop that costs nothing to run.

## What is deliberately NOT built

Automatic, anonymized telemetry (activation rates, promotion frequencies, false-trigger counts) would give stronger signal at scale, but it conflicts with everything above — Wingman is not a hosted service, founders' codebases are sensitive, and standing up collection infrastructure contradicts the lazy-growth, no-speculative-infra stance. If founder-initiated contribution (#1) ever proves too sparse to be useful, revisit it then, opt-in only, and record the decision in `docs/status/PROJECT.md`'s decisions log — not before there's real evidence it's needed.
