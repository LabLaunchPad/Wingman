...
| FR-4 | `/wingman:boardroom` must consolidate the 8 verdicts into one summary with a single bottom line (`GO` / `GO WITH CHANGES` / `DO NOT SHIP`), per the gate rule in `docs/status/ARCHITECTURE.md` §4. | Built |
...
| FR-12 | Every skill must declare an explicit "Use when..." trigger clause in its frontmatter `description`, not only in the body (the "description trap" — see `docs/status/ARCHITECTURE.md` §9, `jeffallan-claude-skills`). | Built, checked by `scripts/validate-structure.mjs` (warning-level) |
...
| FR-14 | (Planned) Department-lead agent files must be created only when their documented activation signal (`docs/status/ARCHITECTURE.md` §5) is actually true for the current project. | Planned |
| FR-15 | (Planned) The MCP state-store server must expose Boardroom checkpoint history as a queryable resource, surviving session boundaries. | Planned — see `docs/status/DATABASE.md` |
...
| NFR-4 | **Lazy agent population.** The always-loaded agent/command/skill surface must stay small regardless of how large the specialist candidate catalog (`docs/roadmap/AGENT-ROSTER.md`) grows — nothing gets built speculatively. |
...
- No LangGraph, no smolagents, no external orchestration service — evaluated and explicitly rejected (see `docs/status/ARCHITECTURE.md` §2, runtime provider decision). Cross-agent collaboration uses Claude Code's own Task-tool dispatch and (where appropriate in the future) the experimental Agent Teams feature, never a custom state-machine runtime.
- The only persistent state Wingman may introduce is the planned local MCP server described in `docs/status/DATABASE.md` — flat files (JSON/JSONL/Markdown), no external database service.
...
- **Skill interface**: auto-triggered by the harness based on `description`; body follows the shape in `docs/status/ARCHITECTURE.md` §6 (When to Use → Core Workflow → Constraints → Rationalizations → Red Flags → Verification → Output).
- **Hook interface**: `hooks/hooks.json` registers lifecycle hooks; `boardroom-checkpoint.mjs` implements the `PermissionRequest`/`ExitPlanMode` contract documented inline in that file and in `docs/status/ARCHITECTURE.md` §9.

## Verification

`plugins/wingman/scripts/validate-structure.mjs` is the mechanical spec-conformance check for this document's structural requirements (FR-12, FR-13, NFR-6). It must exit 0 before any structural change is committed. Behavioral requirements (FR-1 through FR-11) are currently verified by manual walkthrough per change; `docs/status/PROJECT.md` tracks when a lightweight eval harness (per the addyosmani-agent-skills Tier 1/2 pattern referenced in `docs/status/ARCHITECTURE.md`) gets added to check these mechanically as the command/agent surface grows.
