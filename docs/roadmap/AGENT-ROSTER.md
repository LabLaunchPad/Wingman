...
This is the full catalog of specialist roles from Wingman's original enterprise blueprint, organized under the 8 departments defined in `docs/status/ARCHITECTURE.md`. **Every role below is a candidate, not a file.** None of them exist as subagents in a fresh Wingman install.
...
| RL Engineer | Reinforcement-learning system design — for a founder's own project, never for Wingman itself (no RLHF/training layer here; see `docs/status/PRD.md` Non-goals) | candidate |
...
- **Technical (CTO lens)**: 6 of 8 (mindwalk, Context Compressor, Semantic Cache, the Cognitive
  ...
  neither, by deliberate design (`docs/status/ARCHITECTURE.md` §2) — this single architectural fact
...
`docs/history/retros.md`, `docs/status/PROJECT.md`'s decisions log, `docs/HUMAN-TODOS.md`, all 5 real files
...
- **"AI-agent-agnostic transformation"**: genuinely met, but by a different mechanism — see the
  ...
  **Superseded again, 2026-07-27**: the founder came back and asked directly for full
  ...
  absent real demand" stance, not new organic evidence. See `docs/status/ARCHITECTURE.md` §8f for the full
...
- **Context Compressor**: the one "context budget" mention found in `docs/status/PROJECT.md` is about a
...
- **One real, adjacent finding worth noting, not itself a deferred-table row**: `docs/status/PROJECT.md`'s
...
4. Only then is the specialist's agent file written, following `skills/evolve-promotion/references/specialist-agent-template.md`, to `.claude/agents/<specialist-slug>.md` **in the founder's own project — never into Wingman's own plugin directory.** This matches the department-lead rule in `docs/status/ARCHITECTURE.md` §5: plugin files get resynced from the marketplace source, and a specialist tied to one founder's specific codebase belongs in that founder's repo, not baked into a shared plugin install.
