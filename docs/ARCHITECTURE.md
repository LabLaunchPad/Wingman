...
- ~~A third, deliberately adversarial `full-pipeline-e2e` run~~ — done (2026-07-08). An already-committed vulnerable diff (SQL injection + plaintext passwords) handed over as "ship it" produced the first real-independent-dispatch `DO NOT SHIP` (4/5 seats `NO_GO`), promoting `full-pipeline-e2e` to `verified`. All 9 eval cases are now `verified`; the remaining coverage gaps (`learn`/`harness`/`telemetry`, 4 discipline skills) are the deliberately-deferred ones above.
