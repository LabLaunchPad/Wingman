...
- **v21 (6-field skill contract across all 40 skills)**: extended `validate-structure.mjs`'s skill-anatomy check from the 3-concept triad (Rationalizations/Red Flags/Verification) to the full 6-field contract adopted from a founder-supplied blueprint review — Trigger / Inputs / Procedure / Output schema / Stop condition / Escalation. Trigger (frontmatter `description`'s "Use when..." clause) and Procedure ("Core Workflow") were already mechanically covered; the 3 genuinely new fields (Inputs, Output, Escalation) were hand-authored into all 40 skills across 5 waves, each grounded in that skill's own real content rather than templated — a templated pass would have produced 40 skills all saying "Inputs: the task at hand," defeating the point of adopting the contract. `validate-structure.mjs` now emits zero anatomy warnings repo-wide. Fifth of 6 planned PRs harvesting genuine gaps from the founder-supplied blueprint review (see `docs/PROJECT.md`'s decisions log for the full series).
