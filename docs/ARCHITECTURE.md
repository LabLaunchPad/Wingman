...
anywhere). `skills/output/visual-founder-output` closes this the same way `git-pr-workflow` and
...
The mechanism for detecting signals and writing these files is `skills/governance/department-lead-activation`, shared by all six delegating commands rather than duplicated in each.
...
- `/wingman:evolve` (via the `skills/governance/evolve-promotion` mechanism) proposes splitting that sub-task into its own dedicated specialist subagent, names the specific candidate role from the catalog (or a new one if the catalog doesn't cover it), and asks the founder to approve before creating it.

**"Uncapped" means the catalog has no fixed ceiling, not that promotion has no gate.** The 2+-occurrence, founder-approved gate in `skills/governance/evolve-promotion` is unchanged and applies identically regardless of how many candidate rows exist — growing the catalog's *vocabulary* doesn't loosen the *bar* for actually creating one.
...
The 2+-occurrence gate above, and `skills/governance/dogfood-gap-classification`'s decision tree, both need
...
founder installs (only `plugins/wingman/scripts/` ships). This is exactly why `skills/governance/evolve-promotion`
(runs inside *any* founder's installed project) deliberately does **not** depend on it — it falls
back to a plain read/grep of the marker lines instead — while `skills/governance/dogfood-gap-classification`
...
leaving this repo; this layer never does. It's also unrelated to `skills/knowledge/memory`'s
...
- `skills/mechanics/git-pr-workflow` — built on plain `git` + the `gh` CLI, not a platform-specific tool-calling interface; its own `SKILL.md` states this explicitly.
- `skills/mechanics/package-manager-selection` — pure detection logic over `package.json`/lockfiles; contains no Claude-Code tool reference.
...
| `Leonxlnx/taste-skill`, `pbakaus/impeccable`, `nextlevelbuilder/ui-ux-pro-max-skill` | Merged into `skills/output/design-taste` — the countable-rule checklist discipline, the slop-vs-quality/accessibility category split, and a lightweight product-type reference table, respectively. Paid-API and browser-extension sub-skills from these repos explicitly excluded. |
| `DietrichGebert/ponytail` | `skills/discipline/engineering-minimalism`'s decision ladder and "when NOT to be lazy" carve-out. |
| `JuliusBrussee/caveman` | `skills/knowledge/token-economy`, deliberately scoped to internal-only channels — never founder-facing output, which stays governed by `plain-language-checkpoint`. |
| `multica-ai/andrej-karpathy-skills` | Assumption-surfacing and verifiable-success-criteria ideas folded into `skills/discipline/engineering-minimalism`, restated in Wingman's own words (MIT-declared, no standalone `LICENSE` file — not quoted regardless). |
