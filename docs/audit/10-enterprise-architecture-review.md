...
1. **Root-level narrative files with no governance home.** `ANNOUNCEMENT.md`, `AUDIT_REORG.md`, `FIXLOG.md` sit at repo root alongside governance files (`SECURITY.md`, `CONTRIBUTING.md`) but are not indexed in `docs/GOVERNANCE.md`'s cross-reference, and `AUDIT_REORG.md` in particular looks like a stale one-off migration note rather than a durable artifact. This is an ownership-ambiguity violation ("No folder without a clear scope" / "No ambiguous ownership") — nobody's `AGENTS.md` describes what these three files are for or when they get updated.
...
**Phase 1 (immediate, low-risk, do now)**:
- Add an `AGENTS.md`-referenced ownership line for `ANNOUNCEMENT.md` and `FIXLOG.md` (what they're for, update cadence) or fold their content into an existing doc and delete them if stale.
- Determine whether `AUDIT_REORG.md` is a completed migration record (→ move under `docs/audit/` for historical consistency, since that's where dossiers live) or still-active guidance (→ needs an owner line in `docs/GOVERNANCE.md`).
- Add an explicit assertion to `check-repo-consistency.mjs` (or a new tiny script) that no dev-only script pattern exists under `plugins/wingman/scripts/`, and vice versa — closing gap #2 mechanically instead of by convention.
