...
1. Find the matching line in `.wingman/checkpoints.jsonl` by `checkpoint_id` (exact match). If none exists, say so plainly and stop — do not guess at a close match.
...
3. Otherwise, read the file at `details_ref` (`.wingman/checkpoint-details/<checkpoint_id>.md`) and show the full, unabridged `## <SEAT> VERDICT` block(s) — every seat, or just the one named as an optional second argument. This is the same raw detail "Let me see the details" shows live during a fresh run (see "Ask for the decision" below), now retrievable for any past checkpoint, not just the one just reviewed in this session.
