---
name: council
description: Convene a four-voice council for ambiguous decisions, tradeoffs, and go/no-go calls. Use when multiple valid paths exist and you need structured disagreement before choosing.
---

# Council

Convene four advisors for ambiguous decisions:
- the in-context architect voice (your current position)
- a Skeptic subagent
- a Pragmatist subagent
- a Critic subagent

This is for **decision-making under ambiguity**, not code review, implementation planning, or architecture design. For the structured SDLC pipeline, use `wingman:boardroom` instead.

## When to Use

Use council when:
- a decision has multiple credible paths and no obvious winner
- you need explicit tradeoff surfacing
- the user asks for second opinions, dissent, or multiple perspectives
- conversational anchoring is a real risk
- a go / no-go call would benefit from adversarial challenge

Examples:
- monorepo vs polyrepo
- ship now vs hold for polish
- feature flag vs full rollout
- simplify scope vs keep strategic breadth

## When NOT to Use

| Instead of council | Use |
| --- | --- |
| Verifying whether output is correct | `verification-before-completion` |
| Breaking a feature into implementation steps | `writing-plans` |
| Designing system architecture | `wingman:boardroom` (engineer seat) |
| Reviewing code for bugs or security | `wingman:build`'s Definition-of-Done gate |
| Straight factual questions | just answer directly |
| Obvious execution tasks | just do the task |

## Roles

| Voice | Lens |
| --- | --- |
| Architect | correctness, maintainability, long-term implications |
| Skeptic | premise challenge, simplification, assumption breaking |
| Pragmatist | shipping speed, user impact, operational reality |
| Critic | edge cases, downside risk, failure modes |

The three external voices should be launched as fresh subagents with **only the question and relevant context**, not the full ongoing conversation. That is the anti-anchoring mechanism.

## Workflow

### 1. Extract the real question

Reduce the decision to one explicit prompt:
- what are we deciding?
- what constraints matter?
- what counts as success?

If the question is vague, ask one clarifying question before convening the council.

### 2. Gather only the necessary context

If the decision is codebase-specific:
- collect the relevant files, snippets, issue text, or metrics
- keep it compact
- include only the context needed to make the decision

If the decision is strategic/general:
- skip repo snippets unless they materially change the answer

### 3. Form the Architect position first

Before reading other voices, write down:
- your initial position
- the three strongest reasons for it
- the main risk in your preferred path

Do this first so the synthesis does not simply mirror the external voices.

### 4. Launch three independent voices in parallel

Each subagent gets:
- the decision question
- compact context if needed
- a strict role
- no unnecessary conversation history

Prompt shape:

```text
You are the [ROLE] on a four-voice decision council.

Question:
[decision question]

Context:
[only the relevant snippets or constraints]

Respond with:
1. Position — 1-2 sentences
2. Reasoning — 3 concise bullets
3. Risk — biggest risk in your recommendation
4. Surprise — one thing the other voices may miss

Be direct. No hedging. Keep it under 300 words.
```

Role emphasis:
- Skeptic: challenge framing, question assumptions, propose the simplest credible alternative
- Pragmatist: optimize for speed, simplicity, and real-world execution
- Critic: surface downside risk, edge cases, and reasons the plan could fail

### 5. Synthesize with bias guardrails

You are both a participant and the synthesizer, so use these rules:
- do not dismiss an external view without explaining why
- if an external voice changed your recommendation, say so explicitly
- always include the strongest dissent, even if you reject it
- if two voices align against your initial position, treat that as a real signal
- keep the raw positions visible before the verdict

### 6. Present a compact verdict

Use this output shape:

```markdown
## Council: [short decision title]

**Architect:** [1-2 sentence position]
[1 line on why]

**Skeptic:** [1-2 sentence position]
[1 line on why]

**Pragmatist:** [1-2 sentence position]
[1 line on why]

**Critic:** [1-2 sentence position]
[1 line on why]

### Verdict
- **Consensus:** [where they align]
- **Strongest dissent:** [most important disagreement]
- **Premise check:** [did the Skeptic challenge the question itself?]
- **Recommendation:** [the synthesized path]
```

Keep it scannable on a phone screen.

## Persistence Rule

Do **not** write ad-hoc notes to random paths from this skill.

If the council materially changes the recommendation:
- use `wingman:learn` to store the lesson in the right durable location
- or update the relevant issue directly if the decision changes active execution truth

Only persist a decision when it changes something real.

## Multi-Round Follow-up

Default is one round.

If the user wants another round:
- keep the new question focused
- include the previous verdict only if it is necessary
- keep the Skeptic as clean as possible to preserve anti-anchoring value

## Anti-Patterns

- using council for code review
- using council when the task is just implementation work
- feeding the subagents the entire conversation transcript
- hiding disagreement in the final verdict
- persisting every decision as a note regardless of importance

## Related Skills

- `systematic-debugging` — adversarial verification
- `verification-before-completion` — structured verification
- `writing-plans` — gather external reference material before the council if needed
- `engineering-minimalism` — formalize the outcome when the decision becomes long-lived system policy

## Example

Question:

```text
Should we ship Wingman 2.0 as alpha now, or hold until the control-plane UI is more complete?
```

Likely council shape:
- Architect pushes for structural integrity and avoiding a confused surface
- Skeptic questions whether the UI is actually the gating factor
- Pragmatist asks what can be shipped now without harming trust
- Critic focuses on support burden, expectation debt, and rollout confusion

The value is not unanimity. The value is making the disagreement legible before choosing.

## Anti-Rationalization Defense

### Common Rationalizations

| Excuse | Reality |
|---|---|
| "The answer is obvious, I don't need a council" | Obvious answers don't have multiple credible paths. If it's truly obvious, council takes one round and confirms it. If it's not, you needed it. |
| "I'll skip the Skeptic, they'll just slow things down" | The Skeptic is the anti-anchoring mechanism. Skipping them is the rationalization the Skeptic exists to catch. |
| "I already know what the voices will say" | Then the council takes one round and costs almost nothing. If you're right, you've confirmed it. If you're wrong, you've caught it. |
| "I'll feed them the full conversation for context" | Feeding the full conversation to subagents defeats anti-anchoring. They get only the question and relevant context. |
| "I'll skip synthesizing and just pick the pragmatic answer" | The synthesis is where bias guardrails live. Skipping it means you're just hearing what you already believe. |
| "This decision doesn't need persistence" | If the council materially changed the recommendation, it needs to be stored via `wingman:learn`. Decisions that change real execution truth need durable records. |

### Red Flags

- Using council for code review, implementation planning, or architecture design (use the pipeline skills instead).
- Feeding subagents the entire conversation transcript instead of just the question and context.
- Hiding disagreement in the final verdict — strongest dissent must always be visible.
- Skipping the Skeptic voice because they "slow things down."
- Persisting every decision as a note regardless of importance — only persist when it changes something real.
- Forming the Architect position after reading other voices — do it first to avoid anchoring on their output.

### Anti-Pattern Callouts

- **Echo chamber council:** Feeding subagents the full conversation so they anchor on the Architect's position. The anti-anchoring mechanism requires isolation — only the question and relevant context.
- **Verdict laundering:** Synthesizing the council output to match your pre-determined answer while hiding the dissent. The strongest dissent must always be visible.
- **Council-as-procrastination:** Convening a council for an obvious execution task to avoid making a decision. If there's one credible path, just do it.


---

## Harness note: OpenCode (auto-generated by `generate-harness-adapters.mjs` -- do not hand-edit)

This file is a generated copy of the canonical Claude Code source. It references the following Claude-Code-specific mechanism(s); here is the real OpenCode equivalent:

- **ParallelDispatch**: OpenCode confirmed **still sequential**, not genuinely concurrent (open bug anomalyco/opencode#29638, root-caused to `tasks.pop()` + blocking `handleSubtask` in `src/session/prompt.ts`; an experimental `background` param exists but is hidden from the model). Dispatch Boardroom seats one at a time and consolidate the same way this file describes -- disclose this plainly in the founder-facing summary rather than silently pretending parallel review happened.


---

## Harness note: Codex CLI (auto-generated by `generate-harness-adapters.mjs` -- do not hand-edit)

This file is a generated copy of the canonical Claude Code source. It references the following Claude-Code-specific mechanism(s); here is the real Codex CLI equivalent:

- **ParallelDispatch**: Codex CLI has real, confirmed parallel multi-agent dispatch (confirmed 2026-07-27, correcting an earlier "unconfirmed at scale" note): `spawn_agent` creates a sub-agent, `send_message`/`followup_task` directs it, `wait_agent` collects its result, `close_agent` tears it down -- genuinely concurrent, not sequential. Dispatch Boardroom seats the same way this file describes; batch per `max_concurrent_threads_per_session` if this project ever needs more concurrent seats than that session cap allows.


---

## Harness note: Cline (auto-generated by `generate-harness-adapters.mjs` -- do not hand-edit)

This file is a generated copy of the canonical Claude Code source. It references the following Claude-Code-specific mechanism(s); here is the real Cline equivalent:

- **ParallelDispatch**: Cline has no parallel-subagent-dispatch primitive at all -- `/newtask` starts a fresh, sequential context-reset task, not concurrent execution. Dispatch Boardroom seats one at a time in this same session and consolidate the same way this file describes, disclosing the sequential degradation plainly in the founder-facing summary rather than presenting it as a normal parallel review.


---

## Harness note: Cursor (auto-generated by `generate-harness-adapters.mjs` -- do not hand-edit)

This file is a generated copy of the canonical Claude Code source. It references the following Claude-Code-specific mechanism(s); here is the real Cursor equivalent:

- **ParallelDispatch**: Cursor has real, confirmed parallel dispatch: up to 8 background agents, each in its own isolated git worktree. Dispatch Boardroom seats the same way this file describes -- note Cursor batches at 8, so an 8-seat Boardroom (7 core + Design) fits in one batch exactly.


---

## Harness note: Gemini CLI (auto-generated by `generate-harness-adapters.mjs` -- do not hand-edit)

This file is a generated copy of the canonical Claude Code source. It references the following Claude-Code-specific mechanism(s); here is the real Gemini CLI equivalent:

- **ParallelDispatch**: Gemini CLI has real, confirmed parallel subagent dispatch: isolated-context subagents run concurrently and the parent consolidates their results. Dispatch Boardroom seats the same way this file describes.


---

## Harness note: OpenHands (auto-generated by `generate-harness-adapters.mjs` -- do not hand-edit)

This file is a generated copy of the canonical Claude Code source. It references the following Claude-Code-specific mechanism(s); here is the real OpenHands equivalent:

- **ParallelDispatch**: OpenHands has real, confirmed parallel delegation via its `DelegateTool` (newer SDK) -- dispatch Boardroom seats the same way this file describes.
