...
`LEARNINGS.md`, `docs/history/retros.md`, and `.wingman/checkpoints.jsonl` — the three signal sources step 1 reads fresh, never from a remembered impression of this project's history.

## Escalation

Every promotion is itself an escalation to the founder via `AskUserQuestion` (step 4) — nothing is ever written without that explicit approval; there is no further escalation path beyond it.
...
- `docs/history/retros.md` — `## Retro: <title> — <date>` blocks (see `commands/adaptive/retro.md`'s exact format), same marker convention and same fallback rule as above.
- `.wingman/checkpoints.jsonl` — structured, one JSON object per line (see `docs/status/DATABASE.md`'s schema). Group by `seats[].seat` plus repeated keywords in `seats[].summary` or `bottom_line` across multiple lines — e.g. the `ciso` seat (named `security` on any pre-2026-rename `schema_version: 1` entry — same underlying concern, different label; cluster them together, never treat the rename as two different seats) returning `GO_WITH_CONCERNS` about the same narrow topic (auth, a specific integration) more than once is exactly the kind of structured signal this file can surface that free-text sources can't.

**2. Cluster by genuine topical overlap, not superficial keyword match.** Group entries (from any combination of the three sources) that describe the *same underlying friction*, even if worded differently. A cluster is only a candidate if it has **2 or more entries** — this threshold is fixed by `docs/status/ARCHITECTURE.md` §6 and `docs/roadmap/AGENT-ROSTER.md`; do not round down from "close to 2" or promote on a single strong occurrence, no matter how compelling it looks. **Two entries citing the same underlying incident are one occurrence, not two** — check for a shared commit hash, PR number, `checkpoint_id`, or same-day timestamp with matching subject matter before counting a pair as genuinely independent; a single bug logged once via `/wingman:learn` and again via `/wingman:retro` the same session is not repetition.

**3. Classify each qualifying cluster by what it should become**, per `commands/adaptive/evolve.md`'s existing rule. **Check `references/specialist-catalog.md` (in this skill) for a matching row FIRST, before applying the abstract test below** — a catalog match is concrete, evidence-backed prior art for how this exact kind of friction has been classified before, and takes priority over an abstract judgment call that could reasonably go either way (e.g. "migrations need rollback scripts" can sound like a house-style constraint in isolation, but the catalog's own worked example — see its "Using this catalog" section — settles it as the Migration Engineer specialist). Only fall back to the abstract test if no catalog row genuinely fits:
...
**4. Present the proposed promotions to the founder in plain language**, via `AskUserQuestion`: what pattern was noticed, how many times it showed up (cite the actual entries, don't just assert a count), and what it would become. Ask which (if any) to actually create. Do not create anything before this approval. **Harness-aware fallback**: outside Claude Code, check `references/harness-capability-profile.md`'s `hasQuestionTool` column — use the harness's own native question tool if `true` (naming which one), else present the options in prose and treat the next message as the answer. Either way, the approval gate itself is never optional — only the mechanism for asking changes.
