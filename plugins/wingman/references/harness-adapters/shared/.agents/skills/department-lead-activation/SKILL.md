...
The calling pipeline command's name and the project's current codebase/plan state — what the activation-signal check (step 2's table) actually reads to decide.

## Escalation

An ambiguous activation signal (per the Rationalizations table) — look closer or ask the founder rather than defaulting to "create."
...
- Write the file to `.claude/agents/dept-<name>.md` **in the founder's project repository** — never under Wingman's own plugin directory (`plugins/wingman/...`). This distinction matters: see `docs/status/ARCHITECTURE.md` §5 for why.
...
**5. Do the actual delegated work this turn regardless of file-creation timing.** If the department-lead file already existed before this turn, dispatch to it via the Task/Agent tool using its `dept-<name>` agent type as normal. **If the file was just created in this same turn, try that dispatch first, but don't assume it will succeed** — a real dogfood run (2026-07-28) found a harness that rejected the dispatch outright ("Agent type '<name>' not found") immediately after the file was written, because that session's available-agent roster had already been fixed for the turn and didn't rescan `.claude/agents/` mid-turn. Whether this holds in every Claude Code session or was specific to that run's environment is not yet confirmed either way — treat it as a real possibility, not a one-off. **Fallback, if dispatch fails:** do the delegated work directly in this turn yourself, following the just-written department-lead file's own remit/checklist as your operating instructions instead of the subagent's, and tell the founder in one plain-language sentence that the new department lead's file is in place and will be dispatchable as its own subagent starting next session (e.g. "I've added a Product lead to your project; it'll run as its own reviewer from your next session onward — for now I did this turn's work directly"). Never silently skip the delegated work or silently treat a failed dispatch as if it succeeded.
...
- Create all 8 department leads on the first run regardless of signals — this is the exact kitchen-sink anti-pattern `docs/status/ARCHITECTURE.md` and `docs/roadmap/AGENT-ROSTER.md` warn against.
...
| Excuse | Reality |
|---|---|
| "Might as well create all 8 now, saves checking later" | This is the literal kitchen-sink anti-pattern the whole hybrid model exists to avoid — see `docs/status/ARCHITECTURE.md` §2. |
