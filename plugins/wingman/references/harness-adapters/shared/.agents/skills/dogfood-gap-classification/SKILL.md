...
Two real dogfooding passes of Wingman's own 7-stage pipeline (see `docs/history/retros.md`,
...
checkout (see `commands/adaptive/dogfood.md`'s Step 1 mode detection). If you're not sure which one applies,
...
`docs/status/PROJECT.md` decision-log entry if approved):
...
- Append a `## Retro:` entry to `docs/history/retros.md` describing the gap, the classification
  reasoning, and the fix.
- **If the approved change is a hook**: it does not finalize in the same run that found it. Mark it
  `pending-second-opinion` in the `docs/status/PROJECT.md` decision-log entry, and require a second,
  ...
  reverted.
- Add a `docs/status/REGRESSION-CHECKLIST.md` Layer 0 line for the finding, and (once a hook clears
  cooling-off) its own Layer 1 line per that doc's existing graduation rule.
- Record the promotion in `docs/status/PROJECT.md`'s decisions log, including the step-3 safeguard answers
  for any hook.

## Constraints

**MUST:**
- Only run from Wingman's own dev-repo checkout (per `commands/adaptive/dogfood.md`'s mode detection) —
...
approved and implemented: a `docs/status/PROJECT.md` decisions-log entry, a `docs/status/REGRESSION-CHECKLIST.md`
Layer 0 (and, once cleared, Layer 1) line, an eval-case update, and a `docs/history/retros.md` entry.
