...
The thing to audit (a codebase, a PR, a feature) and, if given, the specific concern that triggered thoroughness — a vague "make sure this is good" still needs Step 1's scoping before dispatch.

## Escalation

A finding that's genuinely large or ambiguous (a business tradeoff, an architecturally significant change) gets logged as a durable follow-up and routed to the founder via `plain-language-checkpoint` — never fixed hastily just to close the audit, and never silently dropped either.
