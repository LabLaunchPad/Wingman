...
`.wingman/state.json`'s `active_department_leads` array — the sole source this skill reads to decide whether the conditional-department threshold is crossed.

## Escalation

None — this is a fixed mechanical count (3+ conditionally-activated leads), not a judgment call; there's nothing to hand to the founder short of the plain-language notification in step 3.
