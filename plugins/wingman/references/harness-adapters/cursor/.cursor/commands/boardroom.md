...
- **ExitPlanMode**: Cursor has a real Plan Mode, but the transition out of it is UI-click-only -- no hook fires on it, so there is nothing to intercept (unlike Gemini CLI's coarser-but-real BeforeAgent proxy). Use `plugins/wingman/scripts/install-git-hooks.mjs` (already harness-agnostic, fires under any `git push` regardless of which agent drove the session) as the real enforcement point instead of a mid-session plan gate.
- **ParallelDispatch**: Cursor has real, confirmed parallel dispatch: up to 8 background agents, each in its own isolated git worktree. Dispatch Boardroom seats the same way this file describes -- note Cursor batches at 8, so an 8-seat Boardroom (7 core + Design) fits in one batch exactly.
