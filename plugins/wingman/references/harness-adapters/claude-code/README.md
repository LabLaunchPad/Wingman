...
- **Install path**: `/plugin marketplace add <this-repo>` then `/plugin install wingman@wingman`
  ...
  dogfooding history (see `docs/status/PROJECT.md`'s decisions log and `docs/HUMAN-TODOS.md` for the actual
...
- `AskUserQuestion` — a real interactive multiple-choice UI primitive neither Codex CLI nor OpenCode
  expose an equivalent for; see `docs/status/ARCHITECTURE.md` §8a for the documented fallback.
...
- Native parallel `Task`/`Agent` subagent dispatch for the Boardroom's 7/8-seat review — see
  `docs/status/ARCHITECTURE.md` §8a for how this compares to Codex CLI's confirmed natural-language parallel
