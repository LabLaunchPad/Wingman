...
The bug/failure/unexpected-behavior report, including the exact error message and stack trace if one exists — Phase 1 starts by reading that, not by guessing.

## Output

No fixed template — the outcome is a fix backed by a failing-then-passing test (Phase 4), or, when 3+ fixes have failed, a plain-language architectural finding instead of a 4th fix attempt.
...
Phase 3's "when you don't know" step and Phase 4's 3-failed-fixes threshold are the two escalation points: say "I don't understand X" and research more, or route to the founder/reviewer when it's a business-relevant tradeoff, or when the pattern indicates an architectural problem rather than one more hypothesis to test.
