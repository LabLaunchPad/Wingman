...
The founder-facing content about to be reported, and this session's actual tool surface (whether an Artifact-publishing tool is genuinely available) — the latter is checked fresh every time, never assumed from a prior session.

## Escalation

None beyond `plain-language-checkpoint`'s own — this skill only changes layout, never the underlying verdict; a rendering failure just falls back to Tier B, it doesn't need a founder decision.
...
prose already handles fine. Concretely, in this pass: `commands/pipeline/uxflow.md`'s screen/transition
output, and `commands/adaptive/boardroom.md`'s consolidated report (both seat verdicts and pipeline
