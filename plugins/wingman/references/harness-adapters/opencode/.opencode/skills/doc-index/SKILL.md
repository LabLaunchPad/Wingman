...
The new or promoted `references/*.md` (or other doc) about to enter the tree, and the existing commands/skills/agents that could plausibly own it.

## Output

A cross-linked doc: an owning command/skill/agent's `## References` section citing it by path with a one-line "when to consult" note, plus (if vendor-derived) an `/ATTRIBUTIONS.md` entry.

## Escalation

A `references/*.md` with zero citations anywhere (step 4's audit) — wire it into an owner or delete it; there's no third option of leaving a dead doc in place.
