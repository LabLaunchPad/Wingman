...
A spec or requirements doc for a multi-step task — this skill assumes the spec exists; it doesn't gather requirements itself (see `spec-handler` for that).

## Output

A saved file at `docs/wingman/plans/YYYY-MM-DD-<feature-name>.md`, matching the Plan Document Header + per-task structure below, ending in the required Plain-Language Summary.

## Escalation

Per the Rationalizations table's "vague spec" row: real ambiguity in the spec gets an explicit stated decision in the plan, or gets flagged to the founder — never silently deferred as an implicit placeholder for whoever executes the plan.
