...
- `permissions:` (Agent Permission Model — `read`/`write`/`approve`/`execute`/`deploy`): a pure reviewer/analysis specialist gets `read` only; a specialist that produces code/content gets `read`+`write`. Never `approve` (Boardroom-exclusive) or `deploy` (reserved for `dept-devops`-dispatched work, post-Boardroom-approval only) — this holds even for specialists with `Bash` access, since running commands during work is not the same as deploying or approving. Documentation field for now, checked by `validate-structure.mjs`, not yet runtime-enforced.
