...
- **ExitPlanMode**: OpenCode's real analog is the `plan_exit` tool (confirmed: opencode.ai/docs) plus an experimental, undocumented `OPENCODE_EXPERIMENTAL_PLAN_MODE` flag of unclear strength. The gating logic this canonical file assumes (`boardroom-checkpoint.mjs`'s ExitPlanMode hook) is ported as a real OpenCode plugin at `references/harness-adapters/opencode/.opencode/plugin/wingman-gate.js` -- wire that plugin in rather than re-deriving the gate.
