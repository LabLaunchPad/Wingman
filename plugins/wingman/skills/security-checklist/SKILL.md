...
The actual change being reviewed (diff/files touched) — STRIDE/OWASP are walked against what changed, not the whole codebase from scratch each time.

## Output

The gsd-plugin threat register: one row per concrete risk found, each with a `CLOSED`/`OPEN` disposition and, for `CLOSED`, a cited PR/commit + regression test.

## Escalation

Fix, or escalate to the founder for explicit acceptance, recorded in `docs/wingman/founder-todos.md` — an `OPEN` threat never advances the stage on its own.
