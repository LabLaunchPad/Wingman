...
The discipline skill being written/audited, and — when this fires reactively — the specific excuse an agent just caught itself generating.

## Output

The skill-specific `## Anti-Rationalization Defense` block shown above (Common Rationalizations table + Red Flags + Anti-Pattern Callouts), domain-specific to the skill it's added to. No other output shape.

## Escalation

None — a meta-skill with no founder-facing path. A discipline skill missing this section is a `/wingman:audit` finding against that skill, not something routed to the founder.
