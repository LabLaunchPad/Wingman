...
permissions: {{read for a pure reviewer; read+write for a worker that produces code/content; never approve/deploy — only Boardroom seats approve, only dept-devops-dispatched work deploys, and only after a Boardroom approval}}
---
...
- `permissions:` (Agent Permission Model, see `references/permission-model.md` — `read`/`write`/`approve`/`execute`/`deploy`, mapping onto that doc's Level 0-4 tiers): a pure reviewer/analysis specialist gets `read` only (Level 0); a specialist that produces code/content gets `read`+`write` (Level 2). Never `approve` (Level 3, Boardroom-exclusive) or `deploy` (Level 4, reserved for `dept-devops`-dispatched work, post-Boardroom-approval only) — this holds even for specialists with `Bash` access, since running commands during work is not the same as deploying or approving. Documentation field for now, checked by `validate-structure.mjs`; `deploy-approval-gate.mjs` enforces the Level 3/4 boundary at runtime.
