...
A completed, locally-verified change on a feature branch — this skill starts at "ready to open a PR," it doesn't decide when work is done (that's `verification-before-completion`'s job).

## Escalation

CI red after a real re-diagnosis attempt, a reviewer comment with genuine ambiguity, or anything touching an architecturally significant decision — surface to the founder rather than guessing and pushing another fix blind.
