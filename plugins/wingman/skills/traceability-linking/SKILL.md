...
The requirement/decision/flow/task about to be minted or referenced, and `.wingman/traceability.json`'s current `next_id` counters — read fresh each time, never assumed.

## Escalation

None — this is a mechanical marker convention with a hard verifier (`check-traceability.mjs`); a broken link is an error the script catches, not a judgment call routed to the founder.
