...
The UI surface being built or reviewed, plus the project's own established tokens (palette/font/spacing) if any already exist — new work is checked against those, not decided fresh each time.

## Escalation

Any quality/accessibility-checklist item present (step 5) blocks rather than gets noted — these are defects, not taste calls, and `boardroom-design`'s verdict should reflect that rather than waving them through as style feedback.
