...
- `permissions:` (Agent Permission Model, see `references/permission-model.md` — `read`/`write`/`approve`/`execute`/`deploy`, mapping onto that doc's Level 0-4 tiers): department leads get `read` and `write` (Level 0-2 — they produce work). They never get `approve` (that's the Boardroom's exclusive lane, Level 3) or `deploy` (Level 4, reserved for `dept-devops`-dispatched work, and only after a Boardroom approval has already happened). This is a documentation field for now — it records intent and is checked by `validate-structure.mjs`'s schema validation; `deploy-approval-gate.mjs` separately enforces the Level 3/4 boundary at runtime against checkpoint state, not agent identity.
