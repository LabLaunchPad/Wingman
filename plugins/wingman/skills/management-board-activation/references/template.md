...
- `permissions: read` is deliberate: managers coordinate and recommend, but final `approve` authority stays exclusively with the Boardroom (see the Human Escalation Framework in `commands/boardroom.md`) — a manager fixing a "Medium" concern and re-running the checkpoint is still going through the same gate, not bypassing it with its own approval.
