...
**Before committing any structural change, run both mechanical validators (both must exit 0):** `node plugins/wingman/scripts/validate-structure.mjs` (plugin-internal invariants) and `node scripts/check-repo-consistency.mjs` (repo-root doc/attribution invariants). These are Layer 1 of the regression defense; the genuinely-semantic checks they deliberately don't mechanize are Layer 2, run via `/wingman:audit`. See `docs/REGRESSION-CHECKLIST.md` for the full hybrid and which check lives where.
