...
- **Adaptive commands** (`commands/retro.md`, `learn.md`, `evolve.md`, `harness.md`, `telemetry.md`, `launch.md`, `hotfix.md`, `audit.md`, `over-engineering-review.md`, `bloat-audit.md`, `debt-ledger.md`, `drift.md`, `agents.md`) — invoked as needed, not part of the fixed pipeline. `drift.md` routes mid-flight scope changes back through a real Boardroom checkpoint instead of letting them get silently absorbed into whatever's already being built; `agents.md` is a read-only view of who's currently on the project's AI team. `boardroom.md` also has an opt-in deep-review mode (`/wingman:boardroom deep`) for a second, cross-informed review round — its default single-round path is unchanged.
