...
- **Run 1 — `evals/fixtures/setup-hotfix-fixture.sh`** — "Coupon," a tiny shipped Node discount calculator (`applyDiscount`, zero dependencies, `node --test`). Has a real, deliberately subtle bug: it rounds discounted prices down (`Math.floor`) instead of to the nearest cent (`Math.round`). The 3 existing tests all happen to use price/percent combinations where floor and round agree, so they pass despite the bug — mirroring how a real regression reaches production undetected. `.wingman/checkpoints.jsonl` is pre-seeded with one prior `ship` entry (so `dept-devops`'s "shipped before" signal is true); no `.claude/agents/` exist yet. This is a single-hypothesis bug — the obvious guess is the right one.
- **Run 2 — `evals/fixtures/setup-hotfix-hard-fixture.sh`** — "Cart," a shipped Node cart-totalling module where the *obvious* hypothesis is wrong. The symptom looks like a logic error in `cartTotal`, but the real fault is a module-level `runningTotal` accumulator that's never reset between calls — the classic "passes alone, fails in sequence" order-dependency. Tests that `hotfix` doesn't ship a symptom-fix on the first guess but keeps hypothesizing until the evidence points to the shared state.
...
Run as part of the parallel eval-verification sweep that drove the remaining `provisional` cases to `verified` (see `docs/PROJECT.md`'s decisions log for the sweep and its independent-verification method). Fixture: `evals/fixtures/setup-hotfix-hard-fixture.sh` (the "Cart" module-level shared-state bug above). This scenario specifically tests the failure mode Run 1 couldn't: a bug whose first, obvious hypothesis (the sum logic) is *wrong*, so a correct root-cause pass has to reject that hypothesis on evidence and reach the non-obvious cause (the un-reset module-level accumulator) before fixing. **Result: PASS**, independently verified against real test output — the fix agent did not ship a symptom-patch on the first guess, correctly identified the shared-state root cause, and wrote a real order-dependent regression test confirmed red before green. With Run 1 (single-hypothesis) and Run 2 (multi-hypothesis), the skill is shown to hold across both shapes.
