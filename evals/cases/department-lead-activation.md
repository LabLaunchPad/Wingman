...
### Run 4 — 2026-07-28 (same-turn dispatch-failure fallback, found via `/wingman:dogfood`)

A real maintainer-mode dogfood pass (`evals/fixtures/setup-dogfood-simple.sh`) found Core Workflow Step 5's claim — "dispatch to it via the Task tool... this turn... no plugin reload is needed" — failing outright on first real test: a freshly-created `dept-product.md` in the fixture's `.claude/agents/` could not be dispatched to (`Agent type 'dept-product' not found`). Classified via `dogfood-gap-classification` as a skill-candidate (not a hook — nothing mechanical to enforce, just a false assumption needing a judgment-based fallback) and fixed by adding a try-then-fallback branch to Step 5: attempt dispatch, and if it fails, do the delegated work directly in the same turn using the newly-written file's own remit, telling the founder plainly that the department lead will run as its own subagent from the next session onward.

**Procedure:** reproduced against the same fixture and same `dept-product.md` file. Re-attempted the identical dispatch (confirmed it fails identically, not a fluke) and then followed the new fallback instruction: wrote a real Discovery output (`docs/wingman/discovery/single-job-status.md` in the fixture) directly, following `dept-product.md`'s own remit (requirements analysis scoped to the fixture's actual internal-API stack), rather than silently skipping the delegated work or claiming the dispatch succeeded.

**Result: PASS on the fallback path** — a real, evidence-grounded Discovery output was produced despite the dispatch failure, and the founder-facing sentence the fallback calls for is now a required step, not optional. **Open, honestly disclosed**: whether the underlying dispatch failure is specific to the sandboxed harness this run happened in, or a genuine Claude Code CLI same-turn limitation, is not yet confirmed either way — the fix (a fallback that works regardless of the cause) doesn't require resolving that question, but a future run inside real Claude Code CLI would be worth doing to settle it. `provisional` — this run confirms the fallback branch works once; per this skill's own convention, holding at `provisional` until a second, independently-run scenario reconfirms it (ideally the harness-ambiguity question above too) before promoting to `verified`.
