...
Tests `plugins/wingman/commands/adaptive/dogfood.md` — does its mode-detection correctly distinguish
...
3. Spawn a fresh subagent with only `commands/adaptive/dogfood.md` (not this eval doc), instructed to run in
...
5. Separately, spawn a fresh subagent with only `commands/adaptive/dogfood.md`, instructed to run in
...
| Structured output | `evals/dogfood-runs/<timestamp>-<path>.json` exists, valid JSON, matches the schema in `commands/adaptive/dogfood.md` |
...
Two fresh subagents, each given only `commands/adaptive/dogfood.md` (not this case doc), ran the real
...
A fresh subagent, given only `commands/adaptive/dogfood.md`, ran the full 7-stage pipeline in founder mode
...
post-fix version of `commands/adaptive/dogfood.md` (which now instructs synchronous, non-background
