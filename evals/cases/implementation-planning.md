...
<!-- eval:no-fixture-needed: the distinctive behaviors under test (bundling all 5 planning stages into one task-by-task plan, running the single Planning Milestone Boardroom checkpoint) is the central claim of the pipeline redesign and is exercised end to end by seven-stage-pipeline-e2e.md's two differently-shaped runs — the checkpoint count and bundle shape are confirmed independently in both -->
...
1. Run the `setup-waitlist-app.sh` fixture.
2. Spawn a fresh subagent with `commands/implementation-planning.md` and pre-seeded artifacts from all 4 preceding stages.
