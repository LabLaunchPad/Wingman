...
`provisional` — passed one run covering both the positive (6 managers, correct mapping, correct withholding of the 2 not-yet-active departments) and negative (correctly gated on count, not just per-department mapping) scenarios, independently verified against the real filesystem. Needs a second, differently-shaped scenario (e.g. exactly-3-active-leads boundary case, or a project where a department lead is active but its manager mapping is ambiguous) before promotion to `verified`.
